...
$i_C$ & Indicator function of $C$ & $i_C(\B{x}):= \begin{cases}0,&\mbox{ if }\B{x} \in C\\\infty,&\mbox{ otherwise}\end{cases}$\\
  ...
  $\text{Lap}(\B{w})$ & Laplacian regularization of a 3D image $\B{w}$ & $\frac{1}{2}\|\textbf{Diff }\w\|_\text{Fro}^2 = \frac{1}{2}\sum_{j=1}^p(\textbf{Diff }_x \B{w})_j^2 + (\textbf{Diff }_y \B{w})_j^2 + (\textbf{Diff }_z \B{w})_j^2$ \\ % = \frac{1}{2}{\B{v}}^T\Delta \B{v} \ge 0$
  $\|\B{w}\|_{\text{TV}}$ & Isotropic Total-Variation (TV) regularization & $\|\textbf{Diff }\w\|_{2,1} =
  % \sum_{j}\|(\textbf{Diff }\w)_j\|_2=
