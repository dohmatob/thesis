...
Notation & Name / synopsis & Definition\\ 
  ...
  $\| \B{v} \|_r$ & $\ell_r$ norm for vectors & $\begin{cases}\sqrt[r]{\sum_i |v_i|^r},&\mbox{ if }1 \le r < \infty,\\\max_{i}|v_i|,&\mbox { if }r = \infty\end{cases}$ \\  
...
$ H(X_1) - H(X_2|X_1)$\\
 ...
 $NMI(X_1,X_2)$ & Normalized Mutual Information between two random variables $X_1 \sim p_{X_1}$ and $X_2 \sim p_{X_2}$ &
  $I(X_1,X_2)/\sqrt{H(X_1)H(X_2)}$ \\  
...
$\sigma_C$ & Support function of $C$ & $\sigma_C(\B{x}) := \sup_{\B{z} \in C}\B{x}^T\B{z}$\\
  ...
  $ \dom(f)$ & Effective domain of $f$ & $\{\B{x} \in \mathbb{R}^n | f(\B{x}) < +\infty\}$ \\
