...
Recall from equation \eqref{eq:ss} of chapter \ref{chap:func_var} that after $n$ passes over the data, the unpenalized objective (i.e loss) function whose minimization gives the dictionary updates is $E(\B{D}) := \frac{1}{2n}\|\B{X} - \B{D}\B{C}^T\|_{\text{F}}^2$, where $\B{C} \in \mathbb R^{n \times k}$ is the fixed matrix of codes computed up to this point, and $\B{D} \in \mathbb R^{p \times k}$ is the dictionary variable. Adding a \textbf{block-separable} penalty term $\gamma\sum_{j=1}^kg_j(\B{d}^j)$, the energy becomes
\begin{equation}
...
\section{Conclusion}
These ideas have a great potential to extend the classical dictionary-learning technology providing the practitioner with a modeling framework incorporating a much large class of constraints --namely proximable penalty functions-- than is currently being done.
As regards convergence of our proposed proximal online dictionary-learning scheme, direct application of ~\cite{fercoq2015} seems to suffice, since the general DL algorithm constructs unbiased estimates of $\nabla_j f(\B{D}_t)$, where $f(\B{D}) := \mathbb E_{\B{x}}\min_{\B{c} \in \mathbb R^k}\ell (\B{D}\B{c}, \B{x})$. However, a more careful treatment is warranted, and is left for future work.
