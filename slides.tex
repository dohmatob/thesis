...
\author{\emph{Elvis Dohmatob}\\
  % Phd defense \\
  (Supervised by B. Thirion and G. Varoquaux)}
...
\bigskip
\only<2->{
  \mydot such models are more \emph{intepretable} (i.e small number of \emph{regions})
  }
\end{overlayarea}
\end{frame}
    ...
    \begin{equation*}
      \begin{split}
        \mathcal P(\B{w}) = \begin{cases}
          ...
          \sum_{j \in [\![p]\!]}\rho|\B{w}_j| + \frac{1}{2}(1-\rho)\|(\nabla \B{w})_j\|_2^2, &\mbox{ for GraphNet / S-Lasso
...
\begin{frame}
  ...
  \[
    \underset{\text{\textbf{Explained variance} on test data}}{\includegraphics[width=.36\linewidth]{figures/ev_scores_LANGUAGE.pdf}}
%  \hspace{.05cm}
