...
%% &= \frac{1}{2}\sum_{j \in \mathcal P}\|(\text{diag}(r)\textbf{Diff }
...
%% \sum_{j \in \mathcal P}\|(\text{diag}(r)\textbf{Diff }
%% \B{w})_j\|_2^2 + \gamma_j^{-1}
,
\end{eqnarray}
...
\State \textbf{Recompute scaling:} $\gamma_j^{(k)} \leftarrow (\|(\textbf{Diff }
