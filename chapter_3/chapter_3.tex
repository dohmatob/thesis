...
\section{On the equivalence of TV-L1 and an iteratively-reweighted
  GraphNet model}
...
\sum_{j=1}^p w_j\|(\nabla \B{w})_j\|_2^2 + w_j^{-1},
%% &= \frac{1}{2}\sum_{j \in \Omega}\|(\text{diag}(r)\nabla
%% \B{w})_j\|_2^2 + w_j^{-1},
\\
...
\begin{eqnarray}
\|\B{w}\|_{\text{TV}} = \frac{1}{2}\min_{w \in
  \mathbb R_{++}^p}
\sum_{j=1}^p w_j\|(\nabla \B{w})_j\|_2^2 + w_j^{-1}
%% \sum_{j \in \Omega}\|(\text{diag}(r)\nabla
%% \B{w})_j\|_2^2 + w_j^{-1}
,
\end{eqnarray}
with the optimal weighting vector $w \in \mathbb R_{++}^p$ given by
\eqref{eq:weights}. Whence, the minimizers of the TV-L1 energy
$E_{\text{TV-L1}}(\B{w}) := \ell(\B{w}) +
\alpha \Omega_{\text{TV-L1}}(\B{w})$ coincide with the minimizers of
the iGraphNet energy
\begin{equation}
\begin{split}
E_{\text{GraphNet}}^{w}(\B{w}) &= \ell(\B{w}) +
\alpha \|\B{w}\|_1+\\
&\quad\quad\frac{1}{2}\alpha (1 - \rho)
\sum_{j=1}^p w_j\|(\nabla \B{w})_j\|_2^2
+ w_j^{-1},
\end{split}
\label{eq:rgn}
...
GraphNet model in which per-voxel weights $\alpha(1 - \rho)w_j$ given
by \eqref{eq:weights} replace the constant $\alpha(1 - \rho)$ weight
in the pure GraphNet model \eqref{eq:graphnet}, \eqref{eq:opt_pb}, or
equivalently the $\nabla$ is pre-\textit{whitened} by the diagonal
matrix $W := \mathrm{diag}(\sqrt{w_1},\ldots,\sqrt{w_p})$. This energy
...
\cite{eickenberg2015total} is just an IRLS
