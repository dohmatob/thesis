...
and therefore full-rank (i.e $\hat{\B{D}}\hat{\B{D}}^T$ is invertible).
...
\item Compute the $n_s$-by-$k$ matrix of subject-to-components temporal dynamics $\B{C}_s$
  by regressing the group-level dictionary $\hat{\B{D}}$ onto subject data $\B{X}_s$:
  \begin{equation}
  % \B{X}_s \approx \B{C}_s\hat{\B{D}}.
    \hat{{\B{C}}}_s \in \text{argmin}_{\B{C}_s \in \mathbb R^{n_s \times k}} \|\B{X}_s - \B{C}_s\hat{\B{D}}\|^2_{\text{Fro}}
    \label{eq:dr1}  
    \end{equation}
% \textbf{XXX rather write it: }
  \item Compute individual dictionary $\hat{\B{D}}_s = (\hat{\B{d}}_{s,j}^v)_{1 \le j \le k,1 \le v \le p} \in \mathbb R^{k \times p}$ by regressing the subject's resting-state data $\B{X}_s \in \mathbb R^{n_s \times p}$  onto her subject-to-components temporal dynamics $\hat{\B{C}}_s \in \mathbb R^{n_s \times k}$:
  \begin{equation}
    % \B{X}_s \approx \B{C}_s\B{Z}_s.
    \hat{\B{D}}_s \in \text{argmin}_{\B{D} \in \mathbb R^{k \times p}} \|\B{X}_s - \hat{\B{C}}_s\B{D}\|^2_{\text{Fro}}
    \label{eq:dr2}
  \end{equation}%\textbf{XXX idem}
\end{itemize}
The end result is that for each subject $s$ and each voxel $v$, we
obtain a $k$-dimensional encoding $\hat{\B{d}}_{s,v} \in \mathbb R^k$ of the voxel's time-series $\B{x}_{x,v} \in \mathbb R^{n_s}$ in a common group-level space.
...
$\hat{\B{D}}_s = \hat{\B{C}}_s^\dagger\B{X}_s$ can be rewritten as
\[
  \begin{split}
    \hat{\B{D}}_s  &= (\hat{\B{C}}_s^T\hat{\B{C}}_s)^{-1}\hat{\B{C}}_s^T\B{X}_s
    \\
    &= ((\hat{\B{D}}\hat{\B{D}}^T)^{-1}\hat{\B{D}}\B{X}_s^T\B{X}_s\hat{\B{D}}^T(\hat{\B{D}}\hat{\B{D}}^T)^{-1})^{-1}(\hat{\B{D}}\hat{\B{D}}^T)^{-1}\hat{\B{D}}\B{X}_s^T\B{X}_s\\
    &= \hat{\B{D}}\hat{\B{D}}^T(\hat{\B{D}}\B{X}_s^T\B{X}_s\hat{\B{D}}^T)^{-1}\hat{\B{D}}\B{X}_s^T\B{X}_s\\
    &=
    \hat{\B{D}}\hat{\B{D}}^T\underbrace{\hat{\B{D}}\B{X}_s^T(\B{X}_s\hat{\B{D}}^T\hat{\B{D}}\B{X}_s^T)^{\dagger}\B{X}_s}_{\text{OLS}(\B{X}_s\hat{\B{D}}^T,\B{X}_s)}.
    \end{split}
  \label{eq:mr}
\]
That is, we regress the subject's resting-state time-series data $\B{X}_s$ 
  onto  $n_s$-by-$k$ matrix $\hat{\B{C}}_s := \B{X}_s\hat{\B{D}}^T$ and then
  reweight the result by the component-to-component covariance matrix $\hat{\B{D}}\hat{\B{D}}^T$ of the group-level dictionary. All in all, only a 1 regression step
  is needed.

\subsection{Obtaining the global dictionary $\hat{\B{D}}$}
...
\label{sec:haIsDr}
It turns out that the \textit{shared-response} ``hyper-alignment'' (HA) framework \citep{haxby2011} and the ``dual regression'' (DR) scheme
...
\begin{eqnarray}
  \begin{split}
    &\text{minimize }\frac{1}{N}\sum_{s=1}^N\|\B{X}_s - {\B{C}}_s{\B{D}}\|^2_{\text{Fro}}\\
    &\text{ over }
  {\B{D}} \in \mathbb R^{k \times p},\;{\B{C}}_s \in \mathbb R^{n_s \times k},
  \\
  &\text{ subject to }{\B{C}}_s^T{\B{C}}_s = \Id_k,\; \forall s \in [1\ldots N].
\end{split}
    \label{eq:ha}
\end{eqnarray}
Without the orthonormality constraints ``${\B{C}}_s^T{\B{C}}_s = \Id_k$'',  this problem is precisely the DR problem. \eqref{eq:ha} is usually solved via an alternating minimization scheme. Viz,

\begin{itemize}
\item \textbf{Update rotations (orthonormal Procrustes analysis):}
  $${\B{C}}_s^{(t + 1)} = \U_s^{(t)}{\V_s^{(t)}}^T\; \forall s \in [1\ldots N],
  $$
  where $\U_s^{(k)}\boldsymbol{\Sigma}_s{\V_s^{(t)}}^T$ is the SVD of the $n_s$-by-$k$ matrix $\B{X}_s{\B{D}^{(t)}}^T$.
\item \textbf{Update shared-dictionary:}
  $$\B{D}_s^{(t+1)} = \frac{1}{N}\sum_{s=1}^N\B{D}_s^{(t+1)},
  $$ where $\B{D}_s^{(t+1)} := {\B{C}_s^{(t+1)}}^T\B{X}_s$.
\end{itemize}

However,
\begin{itemize}
\item DR is much more attractive due to its low cost: HA performs an SVD per subject per iteration.
\item HA is usually done parcel-wise (i.e locally) because, the orthonormality conditions are unreasonable globally (i.e full-brain).
\end{itemize}
...
\begin{marginfigure}
  ...
  \label{fig:parcels}
\end{marginfigure}
We now develop our model for predicting subject-specific activation maps $\B{Y}_s$ from resting-state features ${\B{D}}_s$
...
The model \eqref{eq:lrank} can be captured by the following convex program
\begin{equation}
  \begin{split}
...
%%   \tilde{\B{Y}}_{s,l,j} := \B{Z}_s\tilde{\bbeta}_{s,l,j}
%% \end{eqnarray}
...
%   \begin{eqnarray*}
%   \begin{split}
%     \B{Z}_s(\lambda) &= (\A_s^T\A_ss + \lambda \Id_k)^{-1}\A_s^T\B{X}_s \\
