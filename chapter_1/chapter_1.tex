\chapter{Direct EPI-to-EPI inter-subject nonlinear registration}\label{chap:epi2epi}
\markright{{~{\rm \ref{chap:epi2epi}}.Direct EPI-to-EPI inter-subject nonlinear registration}\hfill}{}
...
\caption{\textbf{Nonlinear mismatch} between EPI and T1-weighted image of
...
EPI and the T1-weighted image of the subject (\textbf{Right}). The same native-space
...
of the same subject can be properly aligned to one another via an affine transformation.
...
that push them nonlinearly out-of-match relative to the T1-weighted image of
the same subject. 
%

As an example, Fig. \ref{fig:pb_fig} illustrates this issue (distortions) on the HCP (Human Connectome Project)
...
with severe distortions ~\citep{pmid9178246,pmid12270226,zeng2002,anderson2003}.
%
...
which bypasses the need for a T1-weighted image, and do direct inter-subject
...
~\citep{smith2004} with Normalized Mutual Information as the cost function (option: \textit{-cost
  normmi}).
...
and its T1-weighted image is used as the template. For each other subject,
...
subject's EPI data to the mean thereof.
    The subject's T1-weighted image is then aligned to this
...
\textit{(c)} ANTs is used to learn a deformation of the T1
    ...
    version of the T1-weighted image, together with the corresponding
...
\textit{(d)} the deformation above (T1-based), and all the
    ...
    coregistration) with the T1-weighted image of the subject; these may
...
Then mean of all the registered T1-weighted images is computed, and becomes the template henceforth. This procedure is iterated a couple of times.
...
Connectome Project (HCP) dataset ~\citep{VanEssen20122222}, and a variety
...
using \textit{nipy} ~\citep{Gorgolewski2011}, open-source Python library for analysis of neuro-imaging data. For the purpose of reporting the results, the resulting maps were co-registered to
...
NMI (see Table \ref{Tab:notations} for definition) is a
...
expect the functional activation patterns to be more localized in space
and to have higher peaks. Or could this effect be
...
It is worth noting that there are potentially hundreds of pipelines which could
...
\label{fig:group_zmaps}
\end{figure}

As regards group-level GLM scores, we see from Figure \ref{fig:group_zmaps}
...
%
This
is confirmed in the curves in Figure \ref{fig:group_zmaps}, where we can see that the spatial across-subject activation profiles
...
Classical inter-subject registration pipelines use the T1-weighted (anatomical) image of a
subject to estimate the subject-to-template warp.
%
An obvious issue is that high-quality T1-weighted images are not always
available, but more generally, it is not always possible to completely
align the EPI images of a subject to their T1-weighted image via
...
\begin{pagefigure}
\includegraphics[width=.49\linewidth]{figures/DC+T1_AnteriormedialPFC.png}
\includegraphics[width=.49\linewidth]{figures/DC+SBRef_AnteriormedialPFC.png}
\includegraphics[width=.49\linewidth]{figures/DC+T1_Ventro-medialPFC.png}
\includegraphics[width=.49\linewidth]{figures/DC+SBRef_Ventro-medialPFC.png}
\includegraphics[width=.49\linewidth]{figures/DC+T1_Rightparahippocampalgyrus.png}
\includegraphics[width=.49\linewidth]{figures/DC+SBRef_Rightparahippocampalgyrus.png}
\includegraphics[width=.49\linewidth]{figures/DC+T1_PCC.png}
\includegraphics[width=.49\linewidth]{figures/DC+SBRef_PCC.png}
\includegraphics[width=.49\linewidth]{figures/DC+T1_Leftlateralparietal.png}
\includegraphics[width=.49\linewidth]{figures/DC+SBRef_Leftlateralparietal.png}
\includegraphics[width=.49\linewidth]{figures/DC+T1_Rightlateralparietal.png}
\includegraphics[width=.49\linewidth]{figures/DC+SBRef_Rightlateralparietal.png}
...
%
In terms of more high-level metrics like group-level GLM statistics, these gains though still present, are as not as pronounced (refer to Figure \ref{fig:group_zmaps}). Indeed, as noted in
...
In a separate work ~\citep{dohmatob2016}, also presented in chapter \ref{chap:func_var} in detail, we have considered the possibility of explicitly
