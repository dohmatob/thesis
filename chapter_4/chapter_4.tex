...
e.g functional magnetic resonance imaging (fMRI), ROIs must be
...
\label{sec:intro}
\newthought{In this chapter}, we propose to use the \textit{variability} of the statistical
...
activation. We impose this type of structure via a Laplacian penalty\footnote{This is a slight abuse of language as we really mean the Sobolev semi-norm
$\B{v} \mapsto \B{v}^T \Delta \B{v} = (\nabla \B{v})^T\nabla \B{v}\ge 0$ and not the Laplacian linear operator $\Delta := \nabla^T\nabla$.} on the
...
\begin{figure}
  \centering
  \includegraphics[width=1\linewidth]{smoothdict}  
  \caption{\textbf{Dictionary-learning} with smoothness and sparsity constraints on the atoms. On the right, each time point is a masked 3D brain
    ...
    On the right of the equation, the sought-for $p$-by-$k$ dictionary $\B{D}$  is a low-dimensional representation these images,
    by means of a (non-orthonormal) basis of $k$ $\ll \min(n,p)$ smooth and sparse 3D brain images called \textit{atoms}.
      ...
      is mapped onto $k$-dimensional vector $\B{c}_i$, called the \textit{code} of $\B{X}_i$.
    }
...
The Laplacian regularization $\text{Lap}$ (see Table \ref{Tab:notations} for definition) is meant to impose blobs.
...
$L_{F_{\tilde{\gamma}}(.,\a)} \equiv 1 + \tilde{\gamma} L_{{\text{Lap}}} = 1 + 4D\tilde{\gamma}$, where as before, $D$ is the number of spatial dimensions with $D=3$ for volumetric images. One should also mention that under certain circumstances, it is possible to perform the dictionary updates in the Fourier domain, via FFT. This alternative approach is developed in the Appendix of   ~\citep{dohmatob2016}.
...
For the implementation of the proposed Alg. \ref{Tab:algo}, we
...
To close this section, let us point out a few special instances cases of problem \eqref{eq:constrained_graphnet}, for peculiar choices of the constraint set $Q$. First note that the objective in problem (7) can be conveniently rewritten
as
\begin{equation}
  \begin{split}
...
\end{equation}
a hyper-ellipsoid in standard position (i.e $\mathbf{0}$-centered and axes-aligned). Using elementary geometric arguments, one can show that the projection $P_{\mathcal E}(\hat{\tilde{\B{v}}}^j)$ can be computed efficiently using a kind of root-finding algorithm \citep{Dai06}, and converges exponentially fast.
...
As can be seen in Fig. \ref{fig:maps}, all methods recover
...
others algorithms with about 66\% (resp. 50\% for SODL  ~\citep{mairal2010} and 58\% for Smooth-SODL) of explained variance on the training set, and 60\% (resp. 49\% for SODL and \textbf{55\%} for Smooth-SODL) on left-out (test) data. See Fig. \ref{fig:dictscores}. However, as noted in the above paragraph, such methods lead to dictionaries that are hardly intepretable and thus the user must
% invariably
...
\label{fig:dictscores}
  ...
  Let RAW refer to the direct prediction of targets $\y$ from $\X$, using the top 2000 most voxels most correlated with the target variable. Results of the comparison are shown in Fig. \ref{fig:dictscores}.  Only variables predicted with a positive mean (across the different methods and across subjects) $R$-score are reported.
...
One can envisage to replace the Laplacian regularization with a general structure-inducing penalty for which the proximal operator is easy to compute. Such a framework is developed in chapter \ref{chap:proxdict}, and produces an entire family of models, with potentially different properties.
