...
\begin{split}
  ...
  &=\argmin_{\B{d}^j \in \mathcal C}F_{\gamma (\B{a}_{j,j}/t)^{-1}}(\B{d}^j, \underbrace{\B{d}^j_{\text{old}} + \B{a}_{j,j}^{-1}(\B{b}^j - \B{D}\B{a}^j)}_{\text{see subsection \ref{sec:proxdict} below}}),
\end{split}
\end{eqnarray}
where
...
\subsection{Interlude: Proximal updates for online dictionary-learning}
\label{sec:proxdict}
...
Akin to ICA-type methods, one can take $C_j =$ the orthogonal complement of the linear span of the first $j-1$st atoms, namely
 ...
 \paragraph{``Social'' sparsity:  simultaneous sparsity and smoothness via windowed group-Lasso.}
 ...
 \begin{equation}
   \begin{split}
     ...
     z_\mu\begin{cases}1 - \frac{\alpha}{\|\B{w}_\mu \bullet \B{z}\|_2}, &\mbox{ if }\|\B{w}_\mu \bullet \B{z}\|_2 > \alpha
       % \text{ (i.e the average energy of neighbors of }\mu\text{ is high)}
       ,\\0, &\mbox{ otherwise}\end{cases}\\
     &=
     % z_\mu\left(1 - \frac{\alpha}{\|(\B{w}_\mu)^{1/2} \bullet \B{z}\|_2}\right)_+
...
%  Social sparsity applies a soft-thresholding using the norm of a local neighborhood, but modifies only the coordinate $\B{z}_\mu$ at the center voxel $\mu$ of this neighborhood.
  % It is a generalization of the overlapping group-lasso shrinkage operator\citep{ogrouplasso}; the latter is recovered by taking if the $w^\mu_\nu$ are chosen so that $w^\mu_\nu$ is independent of $\nu$ for all $\nu \in \mathcal N(\mu)$, i.e $w^\mu_\nu = \frac{1}{\#\mathcal N(\mu)}$ for all $\mu$ and $\nu \in \mathcal N(\mu)$.
