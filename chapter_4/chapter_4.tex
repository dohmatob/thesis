...
\begin{equation}
  ...
  \end{equation}
which is by construction, the optimal encoder~\footnote{Under the prior for the codes $p(\B{c}) \propto \exp(-\alpha\phi(\B{c}))$.} for the the generator $G_{\B{D}}$.  One could obtain much greater modeling flexibility by replacing the generator \eqref{eq:lingen} by a neural network
